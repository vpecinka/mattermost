...
## Signature change verification workflow

When I ask you to verify changed method signatures, always follow this order:

1. Identify the interface method and find when it changed using git history (prefer `git log -S '<MethodName>' -- <interface-file>` and inspect the relevant commit diff with `git show`).
2. Search the full codebase for stale implementations and stale call sites (old parameter count/types, old return types, old receiver contracts).
3. If custom implementations in `server/custom/` implement changed interfaces, update them with the smallest compatible change first (no-op methods are acceptable when metrics/behavior is not required).
4. Run a global diagnostics pass to validate interface conformance across the whole repository.
5. Report: (a) commit/date that introduced the signature change, (b) all detected mismatches, (c) exactly what was fixed.

Do not modify `server/enterprise/` while doing this verification/fix workflow.
